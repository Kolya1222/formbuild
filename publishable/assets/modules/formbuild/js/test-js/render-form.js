export function generateMarkup() {
    ...
    let htmlOutput = '';
    let previewOutput = '';
    let paramsOutput = formSender ? 'return [\n' : 'public function setData() {\n';
    ...
    fields.forEach((field, index) => {
        ...
        // Генерация полей (аналогично предыдущему коду)
        switch(type) {
            case 'text':
            case 'email':
            case 'tel':
        ...
        // Генерация правил валидации (как в предыдущем коде)
    ...
    if (hasFileFields) {
        ...
        fileFields.forEach(field => {
            ...
            if (field.accept) {
                rule += `        'allowed' => ['${field.accept.split(',').map(ext => ext.trim()).join("', '")}'],\n`;
            }
            
            if (field.maxSize) {
                rule += `        'maxSize' => ${parseInt(field.maxSize) * 1024},\n`;
            }
